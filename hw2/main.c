...
int main(){
    ...
    int operator_id[6]={0,1,2,3,4,5},operator,new_id,new_salary;
    char new_name[16];
    time_t curtime;
    struct tm *info;
    char buffer[80];
    ...
    FILE* fp_opid;
    if ((fp_opid = fopen("operator_id.bin","r"))==NULL) {
        fp_opid = fopen("records.bin", "wb");
	    fclose(fp_opid);
    }
    ...
    struct emp_record_t emp_record;
    emp_record.emp_id=new_id;
    emp_record.emp_salary=new_salary;
    strcpy(emp_record.emp_name,new_name);
    
    fp_opid =fopen("operator_id.bin","ab");
    fwrite(&emp_record,sizeof(emp_record),1,fp_opid);
    fclose(fp_opid);
    return 0;
}